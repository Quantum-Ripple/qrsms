...
const ALLOWED_ROLES = ['principal', 'admin']
...
const handleLogin = async () => {
  ...
  try {
    ...
    if (!ALLOWED_ROLES.includes(user.role.toLowerCase())) {
      ...
      localStorage.removeItem('token')
      localStorage.removeItem('user')
      error.value = 'Your account does not have access to the Principals portal.'
      loading.value = false
      return
    }
...
<template>
  <div class="min-h-screen flex flex-col justify-center items-center bg-gray-100">
    <div class="bg-white shadow-md rounded-lg p-6 w-full max-w-sm">
      <h2 class="text-xl font-semibold text-center mb-4">Principal Login</h2>

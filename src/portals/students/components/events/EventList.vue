<template>
  <div class="p-6">
    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      <div
        v-for="announcement in filteredEvents"
        :key="announcement.id"
      ...
      >
        ...
        <div class="text-sm text-gray-500 mb-3">
          <p><strong>Date:</strong> {{ formatDate(announcement.created_at) }}</p>
        </div>
...
const filteredEvents = computed(() =>
  events.value.filter(
    e => e.target_audience === 'students' || e.target_audience === 'all'
  )
)
...
function viewDetails(id) {
  router.push({ name: 'StudentEventDetail', params: { id } })
}

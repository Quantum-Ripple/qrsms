<template>
  <div>
    ...
    <div class="flex items-center gap-3 mb-6">
      <div
        class="w-10 h-10 flex items-center justify-center rounded-lg bg-emerald-100 text-emerald-700 text-xl"
      >
        ⚛️
      </div>
      <div>
        ...
        <p class="text-sm text-gray-600">
          Explore forces, energy, motion and more
        </p>
      </div>
    </div>
    ...
    >
      <p class="text-gray-600 text-sm">
        No physics labs available for your class yet.
      </p>
    </div>
...
onMounted(async () => {
  ...
  loading.value = true;
  try {
    simulations.value = await getSimulations("PHY", classLevel);
  } finally {
    loading.value = false;
  }
});
